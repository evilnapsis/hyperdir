...
class PostData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (title,content,address,phone,email,lat,lng,image_id,user_id,use_map,is_public,accept_comments,show_image,created_at) ";
		$sql .= "value (\"$this->title\",\"$this->content\",\"$this->address\",\"$this->phone\",\"$this->email\",\"$this->lat\",\"$this->lng\",$this->image_id,$this->user_id,$this->use_map,$this->is_public,$this->accept_comments,$this->show_image,$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto PostData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set title=\"$this->title\",content=\"$this->content\",address=\"$this->address\",phone=\"$this->phone\",email=\"$this->email\",lat=\"$this->lat\",lng=\"$this->lng\",use_map=\"$this->use_map\",is_public=\"$this->is_public\",accept_comments=\"$this->accept_comments\",show_image=\"$this->show_image\" where id=$this->id";
		Executor::doit($sql);
	}
